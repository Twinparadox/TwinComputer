...
echo("<center><table border=0 style='margin-top:100px;width:1000px'><tr><td align=left>검색된 상품 수 : $total</td><td align=right><a href='goods_show.php?category=$category'>[목록으로]</a></td></tr></table>");
...
if($hit==1) {
	echo("<b><span><a href='search_goods.php?category=$category&hit=1&sorting=-1&field=$field&wSearch=$wSearch'>조회수순</a></span></b>");
}
else {
	echo("<span><a href='search_goods.php?category=$category&hit=1&sorting=-1&field=$field&wSearch=$wSearch'>조회수순</a></span>");
}
echo("<span class='sort_divide_bar'></span>");
if($sorting==0) {
	echo("<b><span><a href='search_goods.php?category=$category&hit=0&sorting=0&field=$field&wSearch=$wSearch'>낮은가격순</a></span></b>");
}
if($sorting!=0){
	echo("<span><a href='search_goods.php?category=$category&hit=0&sorting=0&field=$field&wSearch=$wSearch'>낮은가격순</a></span>");
}	
echo("<span class='sort_divide_bar'></span>");
if($sorting==1) {
	echo("<b><span><a href='search_goods.php?category=$category&hit=0&sorting=1&field=$field&wSearch=$wSearch'>높은가격순</a></span></b>");
}
if($sorting!=1) {
	echo("<span><a href='search_goods.php?category=$category&hit=0&sorting=1&field=$field&wSearch=$wSearch'>높은가격순</a></span>");
}	
echo("</div>");
...
if (!$total){
	echo ("<td align=center><font color=red>검색된 상품이 없습니다</td>");
}
	...
	while ($i<$pagesize) {
		...
		<div style='padding-bottom:5px; padding-top:5px; margin-bottom:15px; text-align:center;'><b style='color:#51afff; font-size:10pt; font-weight:bold;'>$price 원</b></div>
		</div></td>");
		$i++;
	}
	if ($counter%4!=0) {
